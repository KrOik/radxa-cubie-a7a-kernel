...
if [ -f "extracted_a7z/boot-sectors.img" ]; then
    ...
    echo "  Image will NOT be bootable without proper boot0/U-Boot"
    echo "  Run: dd if=extracted_a7z/a7z-stock.img of=extracted_a7z/boot-sectors.img bs=1M count=16"
    echo "  Then write manually: dd if=extracted_a7z/boot-sectors.img of=$OUTPUT_IMAGE bs=1K seek=8 conv=notrunc"
fi
...
echo ""
echo "⚠ Note: This image contains only kernel + modules, no complete rootfs"
echo "   For a bootable system, you need to:"
echo "   1. Extract rootfs from Debian base or stock image"
echo "   2. Copy to partition 3 before compressing"
echo ""
